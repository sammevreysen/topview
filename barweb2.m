function handles = barweb2(barvalues, barmean, errorslow, errorshigh, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, error_sides, legend_type)
...
if nargin < 4
	error('Must have at least the first three arguments:  barweb(barvalues, errorslow, errorshigh, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, barwebtype)');
elseif nargin == 4
	width = 1;
...
if size(barvalues,1) ~= size(errorslow,1) || size(barvalues,2) ~= size(errorslow,2) || size(barvalues,1) ~= size(errorshigh,1) || size(barvalues,2) ~= size(errorshigh,2)
...
% 	end
	if size(barvalues,1) == 1
		...
		errorslow = [errorslow; zeros(1,size(barvalues,2))];
        errorshigh = [errorshigh; zeros(1,size(barvalues,2))];
		change_axis = 1;
	end
	...
	for i = 1:numbars
		...
		x = mean(x([1 3],:));
		handles.errors(i) = errorbar(x, barvalues(:,i), errorslow(:,i), errorshigh(:,i), 'k', 'linestyle', 'none', 'linewidth', 1);
        handles.mean(i) = plot(x,barmean(:,i),'kd');
		ymax = max([ymax; barvalues(:,i)+errorshigh(:,i)]);
	end
